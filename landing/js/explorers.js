...
(function () {
  ...
  const UI = {
    en: {
      skip:'Skip to lesson',brand:'EUA AI Academy',allCourses:'All courses',account:'My learning ↗',title:'AI Explorers',motto:'Human judgment first.',contents:'Course contents',about:'About the academy',start:'Start here',chapters:'The course',library:'Practice & reference',chapter:'Lesson',section:'Section',reading:'min reading · estimate',previous:'← Previous section',next:'Next section →',overview:'Overview',breadcrumb:'Breadcrumb',navigation:'Lesson navigation',retry:'Try again',loadError:'The lesson could not be loaded. Check your connection and try again.',
  ...
  function render() {
    ...
    const hero = section.chapter && first
      ? `<figure class="lesson-hero"><img src="assets/explorers/chapter-${String(section.id).padStart(2,'0')}.png" width="1536" height="864" alt="" loading="eager" decoding="async"></figure>`
      : '';
    $('#lesson').innerHTML = `${learning.position(section,lesson,language,route)}${hero}<div class="lesson-kicker"><span>${section.chapter?`${t('chapter')} ${section.id.padStart(2,'0')}`:['guide-1','guide-2'].includes(section.id)?t('start'):t('library')}</span><span>${lesson.minutes} ${t('reading')}</span></div><h1 class="lesson-title">${esc(lesson.id==='intro'?section.title:lesson.title)}</h1>${first&&section.description?`<p class="lesson-deck">${esc(section.description)}</p>`:''}${welcome}<div class="lesson-body">${body}</div>${visual}${extras}${Number.isInteger(lesson.quiz)?selfCheck(lesson.quiz):''}<div id="chapter-milestone">${learning.ending(section,lesson,data,language,route,completedChapters.has(section.id))}</div>`;
  ...
  async function load(focus = false) {
    ...
    try {
      if (!cache[selectedLanguage]) {
        const response = await fetch(`content/explorers/${selectedLanguage}.json?v=20260915-lecture`);

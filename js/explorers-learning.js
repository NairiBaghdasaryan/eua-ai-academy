/* Authored teaching examples, not live AI calls. Milestones are not assessments. */
(function (root) {
  'use strict';
  ...
  examples.hy = [
    ...
    ['Հանդիպման նշումներից դեպի անելիքների ցանկ', 'Հորինված նշումներ՝ «Անին մինչև երեքշաբթի կազմում է օրակարգը։ Արամը ստուգում է սենյակը։ Միջոցառման ամսաթիվը որոշված չէ»։', ['Անի → Կազմել օրակարգը → Երեքշաբթի', 'Արամ → Ստուգել սենյակը → Ժամկետը նշված չէ', 'Միջոցառման ամսաթիվ → Որոշում է պետք → Պատասխանատուն նշված չէ'], 'Անունները, պատասխանատուներին և ժամկետները համեմատեք նշումների հետ։ Օգտակար աղյուսակը ցույց է տալիս բացերը՝ գուշակություններով լրացնելու փոխարեն։', 'Կազմեք հորինված հանդիպման նշումներ։ Պահանջեք աղյուսակ՝ գործողություն, պատասխանատու, ժամկետ և բաց հարցեր սյունակներով։'],
  ...
  function resultVisual(id, lang, rows) {
    const hy=lang==='hy';
    if (id==='4') {
      const headers=hy?['Պատասխանատու','Գործողություն','Ժամկետ']:['Owner','Action','Deadline'];
      const records=hy?[['Անի','Կազմել օրակարգը','Երեքշաբթի'],['Արամ','Ստուգել սենյակը','Նշված չէ'],['Նշված չէ','Որոշել միջոցառման ամսաթիվը','Նշված չէ']]:[['Ani','Draft agenda','Tuesday'],['Aram','Check room','Not specified'],['Not specified','Decide event date','Not specified']];
      return `<div class="example-table"><table><caption>${hy?'Նշումներից ստացված անելիքները':'Actions extracted from the notes'}</caption><thead><tr>${headers.map(v=>`<th scope="col">${v}</th>`).join('')}</tr></thead><tbody>${records.map(row=>`<tr>${row.map(v=>`<td>${v}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`;
    }
    if (id==='7') return `<figure class="token-comparison"><figcaption>${hy?'Մուտքային թոքենների համեմատություն':'Input token comparison'}</figcaption><div><span>${hy?'Ամբողջ փաստաթուղթ':'Full document'}</span><strong>4,000</strong><i style="--bar:100%" aria-hidden="true"></i></div><div><span>${hy?'Առնչվող հատված':'Relevant excerpt'}</span><strong>1,000</strong><i style="--bar:25%" aria-hidden="true"></i></div><p>${hy?'75% պակաս մուտքային թոքեն։ Երկու տարբերակում էլ՝ 300 ելքային թոքեն։ Որակը պետք է ստուգել առանձին։':'75% fewer input tokens. Both versions use 300 output tokens. Quality must be checked separately.'}</p></figure>`;
    if (id==='6') return `<ol class="example-workflow">${rows.map((row,i)=>{const parts=row.split(' → ');return `<li><span class="workflow-step">${String(i+1).padStart(2,'0')}</span><strong>${parts[0]}</strong><p>${parts[1]}</p>${i===3?`<span class="approval-tag">${hy?'Մարդու որոշում':'Human decision'}</span>`:''}</li>`;}).join('')}</ol>`;
    if (id==='3') return `<ul class="claim-checks">${rows.map((v,i)=>`<li class="${i===0?'supported':'unsupported'}"><span aria-hidden="true">${i===0?'✓':'?'}</span>${v}</li>`).join('')}</ul>`;
    return `<ol>${rows.map(v=>`<li>${v}</li>`).join('')}</ol>`;
  }
  function example(id, lang) {
    const c=copy[lang], e=examples[lang][Number(id)-1];
    if (!e) return '';
    return `<section class="worked-example" aria-labelledby="worked-title"><p class="example-eyebrow">${c.example}</p><h2 id="worked-title">${e[0]}</h2><p class="example-note">${c.note}</p><div class="example-input"><h3>${c.input}</h3><p>${e[1]}</p></div><details class="example-reveal"><summary>${c.reveal}</summary><div class="example-result"><h3>${c.result}</h3>${resultVisual(id,lang,e[2])}</div><div class="example-check"><h3>${c.check}</h3><p>${e[3]}</p></div></details><p class="example-try"><strong>${c.try}:</strong> ${e[4]}</p></section>`;
  }

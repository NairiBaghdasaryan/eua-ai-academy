(function () {
  ...
  const courseHy = {
    ...
    government: ["ԱԲ պետական ծառայության համար", "Հանրային ծառայության մասնագետներ", "Պատասխանատու կերպով կիրառեք ԱԲ-ը հանրային ոլորտի աշխատանքում։"]
  };
  ...
  async function api(path, options = {}) {
    ...
    if (response.status === 401) {
      window.location.href = "/login.html?return=%2Fdashboard.html";
      throw new Error(ui.signIn);
    }
  ...
  function courseAction(course) {
    ...
    if (course.enrollment_status === "payment_pending") {
      return `<div class="dashboard-card-actions"><button class="btn btn-primary" type="button" data-checkout="${course.enrollment_id}">${ui.payment}</button><button class="text-button" type="button" data-promo-open="${course.slug}">${ui.promoQuestion}</button></div>`;
    }
    ...
    return `<div class="dashboard-card-actions"><button class="btn btn-primary" type="button" data-enroll="${course.slug}">${ui.enroll}</button><button class="text-button" type="button" data-promo-open="${course.slug}">${ui.usePromo}</button></div>`;
  }
  ...
  function renderCourses() {
    ...
    container.innerHTML = profile.courses.map((course) => {
      ...
      <article class="dashboard-course-card ${course.enrollment_status === "active" ? "is-enrolled" : ""}">
        ...
        <form class="promo-form hidden" id="promo-${course.slug}">
          <label for="promo-input-${course.slug}">${ui.promoLabel}</label>
          <div><input id="promo-input-${course.slug}" autocomplete="off" maxlength="80" placeholder="${ui.promoPlaceholder}"><button class="btn btn-secondary" type="submit" data-promo-submit="${course.slug}">${ui.apply}</button></div>
          <p class="dashboard-message" id="message-${course.slug}" role="status"></p>
        </form>
        ${courseAction(course)}
      </article>
    `; }).join("");
  ...
  async function enroll(courseSlug, promoCode = "") {
    const message = document.getElementById(`message-${courseSlug}`);
    if (message) message.textContent = ui.processing;
    try {
      const result = await api("/api/enrollments", {
        method: "POST",
        body: JSON.stringify({ course_slug: courseSlug, promo_code: promoCode })
      });
      ...
      if (result.payment_required) {
        ...
        renderCourses();
        const updatedMessage = document.getElementById(`message-${courseSlug}`);
        if (updatedMessage) updatedMessage.textContent = ui.paymentLater;
        return;
      }
      profile = await api("/api/profile");
      renderCourses();
  ...
  function bindCourseActions() {
    ...
    document.querySelectorAll("[data-promo-open]").forEach((button) => button.addEventListener("click", () => {
      document.getElementById(`promo-${button.dataset.promoOpen}`)?.classList.toggle("hidden");
    }));
    document.querySelectorAll("[data-promo-submit]").forEach((button) => button.closest("form").addEventListener("submit", (event) => {
      event.preventDefault();
      const slug = button.dataset.promoSubmit;
      enroll(slug, document.getElementById(`promo-input-${slug}`).value);
    }));
    document.querySelectorAll("[data-checkout]").forEach((button) => button.addEventListener("click", async () => {
      try {
        const result = await api(`/api/enrollments/${button.dataset.checkout}/checkout`);
        window.location.href = result.payment_url;
      } catch (error) {
        const card = button.closest(".dashboard-course-card");
        const message = card.querySelector(".dashboard-message");
        card.querySelector(".promo-form")?.classList.remove("hidden");
        if (message) message.textContent = error.message;
      }
    }));
  }
  ...
  function populateProfile() {
    ...
    document.getElementById("user-avatar").textContent = `${user.first_name?.[0] || parts[0]?.[0] || "E"}${user.last_name?.[0] || parts[1]?.[0] || "U"}`.toUpperCase();

...
const t = language === "hy" ? {
  academy: "← Իմ ակադեմիան", logout: "Ելք", loading: "Դասընթացը բեռնվում է…", course: "ԱԲ հետազոտողներ", courseMeta: "20 դաս · 7 մոդուլ", quizPassed: "✓ Մոդուլի թեստը հանձնված է", quiz: "→ Մոդուլի թեստ", videoSoon: "Տեսանյութը շուտով կավելացվի։", videoAdmin: "Կառավարիչը կավելացնի YouTube-ի հղումը։", previous: "← Նախորդ", next: "Հաջորդ →", finish: "Ավարտել մոդուլը", noQuiz: "Այս մոդուլի համար թեստ դեռ չկա։", quizTitle: "Մոդուլի թեստ", quizIntro: "Հաջորդ մոդուլը բացելու համար հավաքեք առնվազն 70%։", submit: "Ուղարկել թեստը", passed: "Հանձնված է", unlocked: "Հաջորդ մոդուլը բացված է։", notYet: "Դեռ ոչ", need: "Պետք է հավաքեք 70%։ Փորձեք կրկին։", completed: "դասն ավարտված է", of: "-ից", failed: "Հարցումը չհաջողվեց"
} : {
  academy: "← My academy", logout: "Log out", loading: "Loading course…", course: "EUA AI Explorers", courseMeta: "20 lessons · 7 modules", quizPassed: "✓ Module quiz passed", quiz: "→ Module quiz", videoSoon: "Video coming soon.", videoAdmin: "Admin will add the YouTube link.", previous: "← Previous", next: "Next →", finish: "Finish module", noQuiz: "No quiz for this module yet.", quizTitle: "Module quiz", quizIntro: "Pass with 70% or higher to unlock the next module.", submit: "Submit quiz", passed: "Passed", unlocked: "Next module unlocked.", notYet: "Not yet", need: "You need 70%. Try again.", completed: "lessons completed", of: "of", failed: "Request failed"
};
const optionHy = {
...
const armenianize = (value) => String(value || "").replace(/\bAI\b/g, "ԱԲ").replace(/Chatbot/gi, "չաթբոտ").replace(/agent/gi, "գործակալ");
document.documentElement.lang = language;
document.title = language === "hy" ? "ԱԲ հետազոտողներ - Դասընթաց" : "EUA AI Explorers - Course";
...
let viewMode = "lesson"; // lesson | quiz
...
function updateProgress() {
  ...
  document.getElementById("progress-text").textContent = language === "hy" ? `${flat.length}${t.of} ${done} ${t.completed} (${pct}%)` : `${done} ${t.of} ${flat.length} ${t.completed} (${pct}%)`;
}
...
init().catch((error) => {
  window.location.href = error.status === 403 ? "/dashboard.html" : "/login.html";
});
